...
def _install_fake_optional_modules(monkeypatch: Any) -> None:
    ...
    fake_gen.GenerationConfig = GenerationConfig
    fake_gen.GenerativeModel = GenerativeModel
    set_module("vertexai", fake_vertexai)
    set_module("vertexai.generative_models", fake_gen)

    fake_retry = types.ModuleType("retry")
    fake_retry.retry = lambda *args, **kwargs: (lambda fn: fn)
    ...
    fake_torch_utils_data.DataLoader = list
    ...
    fake_transformers.DataCollatorWithPadding = object
...
def test_gpt_judge_reports_missing_provider_configuration(monkeypatch: Any) -> None:
    ...
    fake_openai.AsyncAzureOpenAI = lambda **kwargs: types.SimpleNamespace(**kwargs)
